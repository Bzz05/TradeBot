...
class BinanceDataProducer:
    def __init__(self, config_path):
        ...
        try:
            self.producer = KafkaProducer(
                ...
                retries=5, linger_ms=10)
            logger.info(f"KafkaProducer connected to {kafka_cfg['bootstrap_servers']}")
        except KafkaError as e: 
            logger.error(f"Kafka Producer init failed: {e}", exc_info=True)
            raise
        ...
        self.raw_data_dir = data_paths_cfg["raw_data_directory"]
    ...
    def on_message(self, ws, message):
        """Callback function to handle all incoming messages."""
        try:
            ...
            if 'stream' in msg and 'data' in msg:
                ...
                if data.get('e') == 'kline' and data.get('k', {}).get('x'):
                    ...
                    logger.info(f"LIVE DATA RECEIVED: Symbol={symbol}, TF={timeframe}, Time={kline_open_time.strftime('%H:%M:%S')}, Close={kline['c']}")
                    ...
                    topic_name = f"{self.topic_prefix}{timeframe}-{symbol}"
    ...
    def on_open(self, ws):
        logger.info(f"WebSocket connection opened to {ws.url}")

    def run(self):
        live_futures_stream_base_url = 'wss://fstream.binance.com/stream?streams='
        
        streams = [f"{s.lower()}@kline_{tf}" for s in self.symbols for tf in self.timeframes]
        combined_stream_url = live_futures_stream_base_url + '/'.join(streams)
        
        logger.info(f"Connecting to LIVE Futures WebSocket at: {combined_stream_url}")
        
        ws_app = websocket.WebSocketApp(combined_stream_url,
